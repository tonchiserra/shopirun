...
export const principalScripts = {
    "start": (...flags) => [
        "npx",
        ["shopify", "theme", "dev", "--live-reload=hot-reload", "--theme-editor-sync", "--path=.", "--open", ...flags]
    ],
    "pull-all": (...flags) => [
        "npx",
    ...
    "deploy-staging": null, // Custom handler - deploys to the staging theme, optionally merging every open PR labeled "staging"
    "deploy-all": (...flags) => [
        "npx",
    ...
    "make-backup-and-deploy-without-jsons": (...flags) => [
        "npx",
        ["shopify", "theme", "duplicate", "--force", `--name="[Backup ${getDate()}] | Shopirun"`, ...flags, "&&", "shopify", "theme", "push", "--ignore", "config/settings_data.json", "'templates/*.json'", "'locales/*.json'", "'sections/*.json'", "--path=.", ...flags]
    ],
    "deploy-without-jsons": (...flags) => [
        "npx",
        ["shopify", "theme", "push", "--ignore", "config/settings_data.json", "'templates/*.json'", "'locales/*.json'", "'sections/*.json'", "--path=.", ...flags]
    ],
    "sync-skills": null,    // Custom handler - syncs Claude Code skills from GitHub
    "sync-commands": null   // Custom handler - syncs Claude Code commands from GitHub
}

export const secondaryScripts = {
    "pull-locales": (...flags) => [
        "npx",
        ["shopify", "theme", "pull", "--nodelete", "-o", "'locales/*.json'", "--path=.", ...flags]
    ],
    ...
    "pull-config": (...flags) => [
        "npx",
        ["shopify", "theme", "pull", "--nodelete", "-o", "config/settings_data.json", "--path=.", ...flags]
    ],
